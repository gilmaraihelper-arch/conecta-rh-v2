...
const partnerMenu = [
  ...
  { icon: PlusCircle, label: "Nova Indicação", path: "/indicar" },
  { icon: List, label: "Minhas Indicações", path: "/indicacoes" },
  { icon: DollarSign, label: "Comissões", path: "/comissoes" },
];

const adminMenu = [
  { icon: Shield, label: "Painel Admin", path: "/admin" },
  { icon: List, label: "Todas Indicações", path: "/admin/leads" },
  { icon: Settings, label: "Regras de Comissão", path: "/admin/comissoes" },
        ...
        <div className="flex flex-col items-center gap-8 p-8 max-w-md w-full">
          <div className="flex flex-col items-center gap-6">
            ...
            <p className="text-sm text-muted-foreground text-center max-w-sm">
              Faça login para acessar o painel da Conecta RH.
            </p>
          </div>
          <Button
            onClick={() => {
...
function AuthLayoutContent({
  ...
  const menuItems = useMemo(() => {
    return user?.role === "admin" ? adminMenu : partnerMenu;
  }, [user?.role]);
